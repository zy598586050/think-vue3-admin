				...
				<template #extra>
					<el-alert type="success" :closable="false" class="mb30">
						<template #default>
							...
							<br />
							<div>2、设置国际化：格式：tagsViewName=JSON.stringify({"zh-cn":"测试用","en":"test+page","zh-tw":"測試用"})
							</div>
							<br />
							...
							<div>
								...
								<a href="https://gitee.com/lyt-top/vue-next-admin/pulls/22/files" target="_black">!22 add
									添加自定义 tagVIewName 拓展,支持国际化</a>
							</div>
						</template>
					</el-alert>
					<el-input v-model="tagsViewName" placeholder="请输入tagsView 名称" clearable class="mb15"
...
const { tagsViewName, value, tagsViewNameIsI18n } = toRefs(state)
</script>
