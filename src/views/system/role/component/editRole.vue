    ...
    <el-dialog :title="(formData.id === 0 ? '添加' : '修改') + '角色'" v-model="isShowDialog" width="769px">
              ...
              <el-row :gutter="35">
                ...
                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
                  <el-tree :data="menuData" ref="menuRef" :props="menuProps" :default-checked-keys="formData.menuIds"
                    node-key="id" show-checkbox class="menu-data-tree tree-border" :check-strictly="!menuCheckStrictly" />
                </el-col>
              </el-row>
            </el-form-item>
          </el-col>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="onSubmit" size="default" :loading="loading">{{ formData.id === 0 ? '新 增' : '修改'}}</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
const { isShowDialog, formData, menuExpand, menuNodeAll, loading, menuProps, menuCheckStrictly, menuData, rules } = toRefs(state)
