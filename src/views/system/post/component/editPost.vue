    ...
    <el-dialog v-model="isShowDialog" width="769px">
      <template #header>
        <div v-drag="['.system-edit-post-container .el-dialog', '.system-edit-post-container .el-dialog__header']">
          {{ (formData.postId === 0 ? '添加' : '修改') + '岗位' }}</div>
      </template>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="onSubmit" size="default" :loading="loading">{{ formData.postId === 0 ? '新 增' : '修改'}}</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
const { isShowDialog, formData, rules, loading } = toRefs(state)
