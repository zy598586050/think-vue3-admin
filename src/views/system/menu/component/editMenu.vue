		...
		<el-dialog :title="(acType === 'add' ? '新增' : '修改') + '菜单'" v-model="isShowDialog" width="769px"
				...
				<el-row :gutter="35">
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="上级菜单">
							<el-cascader :options="menuData"
								...
								placeholder="请选择上级菜单" clearable class="w100" v-model="ruleForm.pid">
					...
					<template v-if="ruleForm.menuType === '0' || ruleForm.menuType === '1'">
						...
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
							<el-form-item label="链接地址">
								<el-input v-model="ruleForm.linkUrl" placeholder="外链/内嵌时链接地址（http:xxx.com）" clearable
									:disabled="ruleForm.isLink === 0">
								</el-input>
							</el-form-item>
						</el-col>
					</template>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="权限标识">
							<el-select v-model="ruleForm.roles" multiple placeholder="选择角色" clearable class="w100">
								<el-option v-for="role in roles" :key="'role_' + role.id" :label="role.name"
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="菜单排序">
							<el-input-number v-model="ruleForm.menuSort" controls-position="right" placeholder="请输入排序"
								class="w100" />
						</el-form-item>
					</el-col>
					<template v-if="ruleForm.menuType === '0' || ruleForm.menuType === '1'">
			...
			<template #footer>
				<span class="dialog-footer">
					...
					<el-button type="primary" @click="onSubmit" size="default" :loading="loading">{{ acType === 'add' ? '新 增' : '修改'}}</el-button>
				</span>
			</template>
		</el-dialog>
	</div>
...
import { reactive, toRefs, ref, unref, getCurrentInstance, nextTick } from 'vue';
...
const  { isShowDialog, ruleForm, menuData, rules, roles, loading } = toRefs(state)
