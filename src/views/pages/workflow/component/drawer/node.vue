	...
	<div class="workflow-drawer-node">
		<el-tabs type="border-card" v-model="tabsActive">
			...
			<el-tab-pane label="扩展表单" name="2">
				<el-scrollbar>
					<el-form :model="form" ref="extendFormRef" size="default" label-width="80px"
						class="pt15 pr15 pb15 pl15">
						<el-form-item :label="val.label" :prop="val.prop" v-for="(val, key) in node.from" :key="key"
							...
							<el-select v-model="form[val.prop]" placeholder="请选择" v-if="val.type === 'select'" clearable
								:disabled="val.disabled">
								<el-option v-for="item in val.options" :key="item.value" :label="item.label"
									:value="item.value"> </el-option>
							</el-select>
							<el-checkbox-group v-model="form[val.prop]" v-if="val.type === 'checkbox'"
								:disabled="val.disabled">
...
const { tabsActive, nodeRules, node, form, loading } = toRefs(state)
</script>
